...
YaRN factor-2 profile. The runtime carries the selected profile through RoPE,
model state, fixed-capacity K/V ownership, and persistent cache identity. It
combines persistent exact prefix caches, incremental prefill, Metal prefill
kernels, and exact target-verified MTP or DSpark decode. It does not depend on
DS4 or the previous Ornith 397B
...
The accepted exact scheduler first reached 152.59 tok/s at 128-token prefill and
...
query heads and six positions. For chunks of at least 16 tokens, its value tile
shares each value load across four query heads; chunk 8 and one-query work keep
the occupancy-safe authority. Real layer and complete-model tests remain
bit-exact. Full-model chunk-128 state throughput now improves 31.82% at 65K,

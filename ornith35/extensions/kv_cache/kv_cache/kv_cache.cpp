...
void validate_packed_mse6_append(
  ...
  if (packed.dtype() != mx::uint8 || packed_update.dtype() != mx::uint8 ||
    ...
    throw std::invalid_argument(
        "append_packed_mse6 requires UINT8 payloads and BF16 norms");
  }
  if (packed.ndim() != 3 || packed_update.ndim() != 3 ||
    ...
    throw std::invalid_argument("append_packed_mse6 shape mismatch");
  }
  if (packed_update.shape(1) <= 0 || position < 0 ||
      position > packed.shape(1) - packed_update.shape(1)) {
    throw std::invalid_argument(
        "append_packed_mse6 range is outside capacity");
  }
  if (!packed.flags().row_contiguous ||
    ...
    throw std::invalid_argument(
        "append_packed_mse6 requires row-contiguous arrays");
  }
}
...
std::vector<mx::array> append_packed_mse6(
  ...
  if (packed_keys.shape() != packed_values.shape() ||
    ...
    throw std::invalid_argument("append_packed_mse6 K/V shape mismatch");
  }
  auto primitive = std::make_shared<AppendPackedMSE6>(
...
void AppendPackedMSE6::eval_gpu(
    const std::vector<mx::array>& inputs,
    std::vector<mx::array>& outputs) {
  ...
  auto library = device.get_library(
      "ornith35_kv_cache_ext", current_binary_dir());
  auto kernel = device.get_kernel("ornith35_append_packed_mse6", library);
  auto& encoder = mx::metal::get_command_encoder(stream);
  encoder.set_compute_pipeline_state(kernel);
...
void AppendPackedMSE6::eval_gpu(
  ...
  throw std::runtime_error(
      "Ornith35AppendPackedMSE6 has no Metal implementation");
}
...
void AppendPackedMSE6::eval_cpu(
    const std::vector<mx::array>&,
    std::vector<mx::array>&) {
  throw std::runtime_error("Ornith35AppendPackedMSE6 has no CPU implementation");
}
...
std::vector<mx::array> AppendPackedMSE6::jvp(
    const std::vector<mx::array>&,
    const std::vector<mx::array>&,
    const std::vector<int>&) {
  throw std::runtime_error("Ornith35AppendPackedMSE6 is inference-only");
}

std::vector<mx::array> AppendPackedMSE6::vjp(
    const std::vector<mx::array>&,
    const std::vector<mx::array>&,
    const std::vector<int>&,
    const std::vector<mx::array>&) {
...
std::pair<std::vector<mx::array>, std::vector<int>> AppendPackedMSE6::vmap(
    const std::vector<mx::array>&,
    const std::vector<int>&) {
  throw std::runtime_error("Ornith35AppendPackedMSE6 has no vmap implementation");
}

bool AppendPackedMSE6::is_equivalent(const mx::Primitive& other) const {
  const auto& append = static_cast<const AppendPackedMSE6&>(other);
  return position_ == append.position_;
}

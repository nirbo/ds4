...
def _load_append():
    global _APPEND_BF16, _APPEND_KV_BF16
    if _APPEND_BF16 is not None:
        return _APPEND_BF16, _APPEND_KV_BF16
    extension_root = str(EXTENSION_ROOT)
    if extension_root not in sys.path:
        sys.path.insert(0, extension_root)
    try:
        from ornith35_mlx_kv_cache import append_bf16, append_kv_bf16

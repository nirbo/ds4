...
{
    if (argc != 7 && argc != 8) {
        fprintf(stderr, "usage: %s CATALOG.tsv SHARD_DIR TOKEN_ID LAYERS EXPERT_TOP_K OUT_TOP_K [VOCAB_LIMIT]\n", argv[0]);
        return 2;
    }
    ...
    printf("shards=%zu tensors=%zu layers=%zu token=%llu step_layers=%zu vocab_limit=%zu\n",
           ornith_model_shard_count(model), ornith_model_tensor_count(model), ornith_model_layer_count(model),
           token_id, layers, vocab_limit);
